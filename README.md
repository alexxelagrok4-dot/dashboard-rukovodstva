...
## Список показателей (52 поля ввода)
...
- `NEW_FUNNEL_STAGES` × `PRODUCTS` — воронка «Новые клиенты» (лиды → целевые лиды → КП → заказ), 8 полей. Только для потока «Новые» — у «Повторных»/«Продлений» другая причинная модель (retention, не маркетинг), у «Собрано по долгу» — уже есть отдельный root-cause инструмент (СБ «Продление-Взыскание»).
...
По клику на «Новые клиенты» в разборе выручки открывается воронка лид → целевой лид → КП → заказ, с конверсией между этапами и привязкой каждого этапа к службе (лиды/целевые лиды — маркетинг; КП/заказы — продажи). Автоматический «диагноз недели» сравнивает воронку с предыдущей сохранённой неделей (не с выбранным периодом сравнения — работает всегда) и называет этап с наибольшим падением конверсии и его службу, либо, если конверсии не просели, а упал приток лидов — указывает на маркетинг напрямую.

Разбор утилизации раскладывает недобор тонно/штуко-дней на «потеряно в ремонте» (остаток в ремонте × 7 дней, приблизительная оценка) и остаток — и остаток ведёт в ту же воронку «Новые клиенты», потому что причина обычно одна и та же (дефицит спроса/слабая конверсия), считать её дважды не нужно.
...
Модель осознанно не строит такую же воронку для «Повторных сделок»/«Продлений» — там другая причинная модель (retention: активная база, договоры к продлению, контакт клиентского менеджера), а не маркетинговая воронка; можно добавить отдельно, если понадобится.
